include { starAlignChimeric; runArriba } from '../modules/fusions'
...
workflow fusion_calling {
    ...
    main:
        ...
        //Take optional files and assing dummy file if empty
        ...
        //change to channel.fromFilePairs to get sampleId key
        wgs = params.wgs_sv ? Channel.fromPath(params.wgs_sv).ifEmpty{ file("EMPTY_WGS")} : Channel.fromPath("EMPTY_WGS")

        starAlignChimeric(reads, paired_end, arriba_reference)
        //fusion_bams = Channel.fromFilePairs("/hpc/pmc_vanheesch/projects/Marina/dev/nf_rna_pipeline/test/analysis/arriba/star/*/*.Aligned.out.bam", size: 1)
        ...
        fusion_bams
            ...
            .combine(wgs) //hint for when wgs is avail: .combine(wgs, by: 0)
            .set{ arriba_input }
